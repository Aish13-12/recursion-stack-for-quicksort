...
void printwstack(struct info**head)
{
	...
	for(k=0;k<i-1;k++)
	{
		...
		printf(" (%d,%d)",p->lo,p->hi);//Printing the value from below (of stack)
		if(k==0)
		printf("}");
		printf("\r");//Returning to the start of the line
		for(j=1;j<i-1;j++)
	    printf("\t");//Creating the required identation
		printf("{");
		p=p->addrf;//Going to the next or to the node above the previous node
	}
